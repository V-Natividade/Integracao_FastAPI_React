    ...
    async def register_new_user(self, *, new_user: UserCreate) -> UserInDB:
        if await self.get_user_by_email(email=new_user.email):
            raise HTTPException(
                ...
                detail="Esse e-mail já existe. Faça o login com esse e-mail ou registre-se com outro.",
            )

        if await self.get_user_by_username(username=new_user.username):
            raise HTTPException(
                status_code=HTTP_400_BAD_REQUEST, detail="Esse nome de usuário já está em uso. Por favor, tente outro."
            )
